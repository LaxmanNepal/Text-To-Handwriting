...
const APP_SHELL = [
  ...
  `${APP_SCOPE}offline.js`, `${APP_SCOPE}documents.js`, `${APP_SCOPE}icons/icon.svg`
];
...
self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==self.location.origin||!u.pathname.startsWith(APP_SCOPE))return;if(e.request.mode==='navigate'){e.respondWith(fetch(e.request).then(r=>save(e.request,r)).catch(()=>caches.match(e.request).then(r=>r||caches.match(`${APP_SCOPE}index.html`))))}else e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).then(x=>save(e.request,x))))});
