...
def fetch(url):
    req = Request(url, headers={'User-Agent':'TextToHandwriting-Offline-Bundler/2.0'})
...
def vendor(url, kind='asset'):
    ...
    if kind == 'css' or 'css' in ctype or target.suffix.lower() == '.css':
        ...
        def css_asset(m):
            ...
            try: return 'url("'+vendor(absolute)+'")'
            except Exception as e:
                raise RuntimeError(f'Cannot vendor CSS asset {absolute}: {e}')
...
def patch_html():
    path = OUT/'index.html'
    if not path.exists(): raise RuntimeError('index.html not found')
...
def verify():
    ...
    for p in OUT.rglob('*'):
        if p.is_file() and p.suffix.lower() in {'.html','.css','.js','.webmanifest'}:
            ...
            if re.search(r'https?://',text): bad.append(str(p.relative_to(OUT)))
    if bad: raise RuntimeError('External URLs remain in runtime assets: '+', '.join(bad))
